...
전자세금계산서 매출·매입 목록, 기간 합계, 상세 및 등록 거래처 조회·관리를 지원합니다.
0.4.0은 일반 단건 발행·기재사항 정정·전액 취소의 미리보기 및 확인 전송 API를 추가합니다.
발행 전송은 기본 비활성이며 실제 홈택스 발행 호환성은 미검증입니다.
거래처 등록·수정·삭제는 기본 미리보기이며, 실제 반영에는 별도 서버 설정과 명시적 확인이 필요합니다.
...
거래처 등록·수정·삭제: [거래처 관리 API](docs/COUNTERPARTIES.md).
세금계산서 단건 발행·정정·취소: [발행 API와 검증 한계](docs/ISSUANCE.md).
실제 등록·수정·삭제는 이 구현 작업에서 실행하지 않았으며, 모의 응답으로만 검증합니다.
