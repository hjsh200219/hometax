...
검증된 세션을 서버 메모리에서 관리하는 독립 Python API입니다. 세금계산서 발행 기능은 없습니다.

**검증 상태:** macOS Apple Silicon에서 합성 인증서 암복호화·서명, HTTP API, 단위 테스트를
확인했습니다. 홈택스의 공개 챌린지 응답도 확인했습니다. 실제 인증서를 사용한 홈택스
로그인 성공은 아직 검증하지 않았습니다. 홈택스가 공식 제공하는 개발자 로그인 API가
...
| 형식 | 파일 검증 | 홈택스 로그인 |
|---|---|---|
| NPKI `signCert.der` + `signPri.key` | 지원 | 개인키의 VID `randomNum`이 있으면 요청 가능, 실인증 미검증 |
...
로그인 요청에는 `login_type`을 추가합니다. 공개 참고 구현에 `03`과 `04`가 사용되며,
계정·인증서 종류별 의미와 호환성을 실제 홈택스 화면으로 확인해야 합니다. 기본값이나
