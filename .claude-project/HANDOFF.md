...
project: hometax
summary: 외부 보고서 F01-F05 수정과 cryptography 50.0.1 갱신, 전체 343개 테스트 통과. 변경은 로컬이며 미커밋·미푸시
---

## Session Digest

### 최신: 외부 기능·보안 보고서 결함 수정 (2026-09-12)

- 기준 `4f4c045`, 작업 전 fetch 및 clean 상태 확인.
- F01/F05: 조회 기간 역전·미래 날짜/연도·잘못된 날짜를 로그인 전에 사용 오류(exit 2)로 거절.
- F04: `--json`을 하위 명령 전후 모두 허용. 기존 위치의 값도 유지.
- F02: CLI 발행·정정·취소의 `--yes`는 선택 인증서로 새로 로그인한 뒤 미리보기/전송.
  쿠키 캐시에 인증서 지문을 추가하지 않았으며 일치 검사를 그대로 유지.
- F03: CLI 신규 발행 ID는 검증·정규화 요청으로 결정. 저널 v2 내용 가드는 동일 사업자의
  동일 내용에 미확정 전송이 있으면 새 ID도 차단. 동일 ID 완료 재전송 차단 유지.
- v1 저널은 기록을 보존하며 테이블만 추가. 기존 미확정 기록은 내용 대조 불가하므로
  신규 발행을 보수적으로 차단. 저널 삭제나 완료 상태 추정으로 해제하지 말 것.
- cryptography 48.0.1 → 50.0.1. 합성 NPKI·PFX·XML 서명 호환 테스트 통과.
- 외부 socket 차단 fixture 추가. 전체 343개 테스트·Ruff·format·compileall·wheel/sdist 통과.
- 독립 리뷰 blocker 0건. 실제 로그인·발행·정정·취소를 사용한 통합 검증은 이번 수정에서 미수행.
- 변경은 로컬 작업 트리에 남김. 커밋·push·배포는 하지 않음.
