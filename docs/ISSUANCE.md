...
- 신규 발행의 `client_reference`는 **같은 발행 의도에 대해 항상 같은 UUID**를 재사용해야 합니다.
  CLI에서 생략하면 검증·정규화한 요청 내용으로 안정적인 UUID를 계산합니다. 같은 입력은
  프로세스를 다시 실행해도 같은 ID를 사용합니다. 전송 전에 기존 저널에 기록됩니다.
  명시적인 새 UUID를 사용해도 같은 사업자의 동일 요청 내용이 `in_flight`/`unknown`이면
  원자적 내용 가드가 재전송을 막습니다. 완료 결과가 확인된 별개 거래만 새 ID로 구분하세요.
...
CLI의 `--yes` 발행·정정·취소는 선택한 인증서로 한 번 새로 로그인한 뒤 미리보기를 생성합니다.
쿠키만 복원한 조회 세션이나 파일에 적힌 지문을 서명 인증 근거로 신뢰하지 않습니다.
비밀번호·개인키를 저장하지 않으며 기존 인증서 일치 검사를 유지합니다.

저널 v1은 기존 기록을 보존하며 v2 내용 가드 테이블을 추가합니다. 과거 미확정 기록에는
내용 해시가 없어 같은 거래인지 대조할 수 없으므로, 그런 기록이 있으면 신규 발행을
보수적으로 차단합니다. 자동 재전송·기록 삭제·완료 상태 추정은 하지 않습니다.
