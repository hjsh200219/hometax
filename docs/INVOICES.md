...
이 문서는 조회 기능을 다룹니다. 발행·정정·취소는 [발행 API](ISSUANCE.md)를 참고하세요.
면세 전자계산서, PDF/엑셀 다운로드는 포함하지 않습니다.
...
홈택스가 상호를 비워 보내는 행이 있어 `counterparty_name`은 `null`일 수 있습니다.
실제 매입 목록에서 확인했으며, 이때 대체 상호 필드(`splrTnmNm`·`dmnrTnmNm` 등)도 비어 있습니다.
상호가 문자열이 아닌 값으로 오면 응답 변경으로 보고 거절합니다.
